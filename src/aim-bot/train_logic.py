...
from brain import ActorCritic, PPOAgent
import time
import os
import wandb
import argparse
...
def train(resume_path=None):
    # 0. Init WandB
    run_name = f"aimbot_math_{int(time.time())}"
    wandb.init(
        project="ppo-aimbot",
        name=run_name,
        config={
    ...
    agent = PPOAgent(model, learning_rate=LR)
    ...
    while step < TOTAL_TIMESTEPS:
        ...
        if step % SAVE_INTERVAL == 0:
            save_path = f"weights/model_{step}.npz"
            model.save_weights(save_path)
            print(f"Checkpoint saved: {save_path}")
            
    # Save Final
    model.save_weights("weights/best_aim.npz")
    print("Training Complete. Weights saved to weights/best_aim.npz")
    wandb.finish()
